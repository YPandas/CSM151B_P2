...
uint8_t inline crc_8( const char *input_str, size_t num_bytes ) {
...
uint8_t inline crc_hash(uint8_t bhr, uint8_t pc_index) {
	char total[2];
    total[0] = bhr; total[1] = pc_index;
    return crc_8(total, 2);
}

#define HASH_FUNC xor_hash
#define HASH_FUNC_NAME "xor"
